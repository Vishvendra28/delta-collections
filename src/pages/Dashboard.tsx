...
const uniqueCustomers = new Set(filtered.map(t => t.customer)).size;
  ...
  const periodGroups = useMemo(() => {
    const p1 = allDates.filter(d => new Date(d + 'T00:00:00').getDate() <= 10);
    const p2 = allDates.filter(d => { const day = new Date(d + 'T00:00:00').getDate(); return day >= 11 && day <= 20; });
    const p3 = allDates.filter(d => new Date(d + 'T00:00:00').getDate() >= 21);
    return [p1, p2, p3].filter(g => g.length > 0);
  }, [allDates]);

  const visibleDates = useMemo(() => {
    if (!showAllDates) return allDates.slice(-3);
    return periodGroups[datePage] ?? [];
  }, [showAllDates, allDates, periodGroups, datePage]);

  const totalPages = periodGroups.length;

  const periodLabel = useMemo(() => {
    if (!showAllDates || !periodGroups[datePage]?.length) return '';
    const dates = periodGroups[datePage];
    const first = new Date(dates[0] + 'T00:00:00');
    const last  = new Date(dates[dates.length - 1] + 'T00:00:00');
    const mon   = first.toLocaleString('en-IN', { month: 'short' });
    return `P${datePage + 1} · ${mon} ${first.getDate()}–${last.getDate()}`;
  }, [showAllDates, periodGroups, datePage]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div className="card" style={{ display: 'flex', padding: 0, overflow: 'hidden' }}>
        ...
        {/* Left: MTD Collection */}
        <div style={{ flex: 1, padding: '10px 18px', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 3 }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text2)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Total MTD Collection</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--brand)', lineHeight: 1.1 }}>{formatCrRounded(mtdTotal)}</div>
          <div style={{ fontSize: 11, color: 'var(--text3)', display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap' }}>
            ...
            {momDelta !== null && (
              <span style={{ fontWeight: 700, color: momDelta >= 0 ? 'var(--success)' : 'var(--danger)', fontSize: 10.5, background: momDelta >= 0 ? '#d1fae5' : '#fee2e2', padding: '1px 6px', borderRadius: 99 }}>
        ...
        <div onClick={() => onNavigate('target')}
          style={{ flex: 1, padding: '10px 18px', cursor: 'pointer', display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 5 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <span style={{ fontSize: 10, fontWeight: 700, color: 'var(--text2)', letterSpacing: '0.08em', textTransform: 'uppercase' }}>Target vs Actual</span>
          ...
          {targetTotal > 0 ? (
            <>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                <div>
                  <div style={{ fontSize: 9.5, color: 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', marginBottom: 1 }}>Target</div>
                  <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--text)' }}>{formatCrRounded(targetTotal)}</div>
                </div>
                <div style={{ width: 1, height: 24, background: 'var(--border)', flexShrink: 0 }} />
                <div>
                  <div style={{ fontSize: 9.5, color: 'var(--text3)', fontWeight: 600, textTransform: 'uppercase', marginBottom: 1 }}>Actual</div>
                  <div style={{ fontSize: 17, fontWeight: 800, color: 'var(--brand)' }}>{formatCrRounded(mtdTotal)}</div>
                </div>
                {(() => {
                  const pct = (mtdTotal / targetTotal) * 100;
                  const color = pct >= 80 ? 'var(--success)' : pct >= 50 ? '#d97706' : 'var(--danger)';
                  const bg    = pct >= 80 ? '#d1fae5' : pct >= 50 ? '#fef3c7' : '#fee2e2';
                  return <span style={{ marginLeft: 'auto', fontSize: 13, fontWeight: 700, color, background: bg, padding: '2px 8px', borderRadius: 99 }}>{pct.toFixed(1)}%</span>;
                })()}
              </div>
              {(() => {
                const pct = (mtdTotal / targetTotal) * 100;
                const color = pct >= 80 ? 'var(--success)' : pct >= 50 ? '#d97706' : 'var(--danger)';
                return <div style={{ height: 4, borderRadius: 99, background: 'var(--border)' }}><div style={{ height: '100%', borderRadius: 99, background: color, width: `${Math.min(pct, 100)}%`, transition: 'width 0.4s' }} /></div>;
              })()}
            </>
          ) : (
            <div style={{ fontSize: 12, color: 'var(--text3)' }}>No target uploaded for {monthLabel(targetMonth)}</div>
          )}
        </div>
          ...
          <table>
                ...
                {visibleDates.map(d => {
                  ...
                  return (
                    <th key={d}
                      ...
                      style={{
                        textAlign: 'right', minWidth: showAllDates ? 95 : 140,
            ...
            <tbody>
              {customerRows.map(([customer, data]) => (
                <tr key={customer}>
                  <td style={{ fontWeight: 600, ...(showAllDates && { padding: '5px 10px' }), position: 'sticky', left: 0, background: 'var(--surface)', zIndex: 1, fontSize: showAllDates ? 13 : 14, borderRight: showAllDates ? '1px solid var(--border)' : undefined }}>{customer}</td>
                  <td style={{ color: 'var(--text2)', fontSize: showAllDates ? 12 : 13, ...(showAllDates && { padding: '5px 8px' }), borderRight: '2px solid var(--border)' }}>{data.kam}</td>
                  {visibleDates.map(d => (
                    <td key={d} style={{
                      textAlign: 'right', ...(showAllDates && { padding: '5px 8px', fontSize: 13 }),
                      borderRight: '2px solid var(--border)',
                  ...
                  <td style={{ textAlign: 'right', ...(showAllDates && { padding: '5px 10px' }), fontWeight: 800, color: 'var(--brand)', fontSize: showAllDates ? 13 : 14 }}>{formatCrRounded(data.total)}</td>
                </tr>
              ))}

              {customerRows.length > 0 && (
                <tr style={{ background: 'var(--surface2)' }}>
                  <td style={{ fontWeight: 800, fontSize: showAllDates ? 13 : 14, ...(showAllDates && { padding: '5px 10px' }), position: 'sticky', left: 0, background: 'var(--surface2)', borderRight: showAllDates ? '1px solid var(--border)' : undefined }}>Grand Total</td>
                  <td style={{ ...(showAllDates && { padding: '5px 8px' }), borderRight: '2px solid var(--border)' }} />
                  {visibleDates.map(d => (
                    <td key={d} style={{ textAlign: 'right', ...(showAllDates && { padding: '5px 8px', fontSize: 13 }), fontWeight: 700, borderRight: '2px solid var(--border)' }}>
                      {dateTotals[d] ? formatCrRounded(dateTotals[d]) : '—'}
                    </td>
                  ))}
                  <td style={{ textAlign: 'right', ...(showAllDates && { padding: '5px 10px' }), fontWeight: 800, color: 'var(--brand)', fontSize: showAllDates ? 13 : 15 }}>{formatCrRounded(mtdTotal)}</td>
                </tr>
              )}
